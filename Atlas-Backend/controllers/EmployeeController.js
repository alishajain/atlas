...
const addEmployee = async (req, res) => {
    const {
        EmpId, EmpName, Address, EmailId, ContactNo, AltContactNo, DOB, Age, JoiningDate, Designation, Anniversary
    } = req.body;

    // Basic validation: Ensure all required fields are provided
    if (!EmpId || !EmpName || !EmailId || !ContactNo || !DOB || !JoiningDate || !Designation) {
    ...
    try {
        ...
        res.status(201).json({ success: true, message: 'Employee added successfully', data: { EmpId, EmpName } });
    } catch (err) {
...
const deleteEmployee = async (req, res) => {
    ...
    try {
        ...
        const result = await db.query("DELETE FROM employee_details WHERE EmpId = ?", [empId]);
...
const updateEmployee = async (req, res) => {
    ...
    try {
        const result = await db.query(
            ...
            [
                EmpName, Address, EmailId, ContactNo, AltContactNo, DOB, Age, JoiningDate, Designation, Anniversary, empId
            ]
        );

...
const addYarnStockDetails = async (req, res) => {
  const { BillDate, YarnId, SupplierName, SupplierCity, Weight, Amount, BillNo, UserId, LotNo } = req.body;


  // Validate input fields
  if (!BillDate || !YarnId || !SupplierName || !SupplierCity || !Weight || !Amount || !BillNo || !UserId || !LotNo) {
  ...
  try {
    ...
    const [existingStock] = await connection.query(
      "SELECT 1 FROM yarn_inventory WHERE YarnId = ? AND BillDate = ? AND LotNo = ?",
      [YarnId, BillDate, LotNo]
    );
    ...
    const [newYarnStockDetails] = await connection.query(
      "INSERT INTO yarn_inventory (BillDate, YarnId, SupplierName, SupplierCity, Weight, Amount, BillNo, UserId, LotNo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [BillDate, YarnId, SupplierName, SupplierCity, Weight, Amount, BillNo, UserId, LotNo]
    );

...
const addKachian = async (req, res) => {
  try {
    ...
    db.query(query, values, (error, results) => {
      if (error) {
        console.error(error);
        return res.status(500).json({ message: 'Error inserting data into the kachian table.' });
      }
...
const getKachianByRSN = (req, res) => {
  ...
  db.query(query, [RSN], (error, results) => {
    if (error) {
      console.error(error);
      return res.status(500).json({ message: 'Error fetching data from the kachian table.' });
    }

...
// Controller to add data into the mending table
const addMending = async (req, res) => {
  try {
    ...
    const query = `
      INSERT INTO mending (RSN, EmpID, Cost, UserId)
      VALUES (?, ?, ?, ?);
    `;
    ...
    db.query(query, values, (error, results) => {
      if (error) {
        console.error(error);
        return res.status(500).json({ message: 'Error inserting data into the mending table.' });
      }
...
// Controller to fetch data by RSN from the mending table
const getMendingByRSN = (req, res) => {
  ...
  db.query(query, [RSN], (error, results) => {
    if (error) {
      console.error(error);
      return res.status(500).json({ message: 'Error fetching data from the mending table.' });
    }

...
const addCutting = async (req, res) => {
  try {
    ...
    db.query(query, values, (error, results) => {
      if (error) {
        console.error(error);
        return res.status(500).json({ message: 'Error inserting data into the cutting table.' });
      }
...
const getCuttingByRSN = (req, res) => {
  ...
  db.query(query, [RSN], (error, results) => {
    if (error) {
      console.error(error);
      return res.status(500).json({ message: 'Error fetching data from the cutting table.' });
    }

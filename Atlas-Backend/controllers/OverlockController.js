...
const addOverlock = async (req, res) => {
  try {
    ...
    db.query(query, values, (error, results) => {
      if (error) {
        console.error(error);
        return res.status(500).json({ message: 'Error inserting data into the overlock table.' });
      }
...
const getOverlockByRSN = (req, res) => {
  ...
  const query = `
    SELECT * FROM overlock WHERE RSN = ?;
  `;
  ...
  db.query(query, [RSN], (error, results) => {
    if (error) {
      console.error(error);
      return res.status(500).json({ message: 'Error fetching data from the overlock table.' });
    }

...
// Controller to add data into the thoketanke table
const addThokeTanke = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      success: true,
      message: 'ThokeTanke data added successfully',
      data: result,
    });
  } catch (error) {
    ...
    return res.status(500).json({
      success: false,
      message: 'Error adding thoketanke data',
    });
  }
};

// Controller to fetch data by RSN from the thoketanke table
const getThokeTankeByRSN = async (req, res) => {
  const { RSN } = req.params;

  try {
    const [rows] = await db.query('SELECT * FROM thoke_tanke WHERE RSN = ?', [RSN]);

    if (rows.length === 0) {
      return res.status(404).json({ success: false, message: "ThokeTanke record not found." });
    }

    res.status(200).json({ success: true, data: rows[0] });
  } catch (err) {
    console.error("Error fetching thoketanke details:", err);
    res.status(500).json({ success: false, message: "Error fetching thoketanke details.", error: err.message });
  }
};

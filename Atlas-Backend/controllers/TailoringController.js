...
const addTailoring = async (req, res) => {
  try {
    ...
    db.query(query, values, (error, results) => {
      if (error) {
        console.error(error);
        return res.status(500).json({ message: 'Error inserting data into the tailoring table.' });
      }
...
const getTailoringByRSN = (req, res) => {
  ...
  const query = `
    SELECT * FROM tailoring WHERE RSN = ?;
  `;
  ...
  db.query(query, [RSN], (error, results) => {
    if (error) {
      console.error(error);
      return res.status(500).json({ message: 'Error fetching data from the tailoring table.' });
    }
